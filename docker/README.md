...
- docker build -t base-multichain ./base

Master node:

- docker build -t joshuawithey/master-node:latest ./master
- Pushing to azure: https://docs.docker.com/cloud/aci-integration/
- docker login azure
- docker context create aci myacicontext
- docker --context myacicontext run -p 7557:7557 -p 8002:8002 joshuawithey/master-node:latest

Child node:

- docker build -t joshuawithey/slave-node:latest ./slave
- docker run -P --name <node_name> -e MASTER_NODE=<IP of master node> joshuawithey/slave-node:latest
...
- PARAM\_<something descriptive>='<variable>|<value>' e.g: `PARAM_TARGET_BLOCK_SIZE='target-block-time|15'`
